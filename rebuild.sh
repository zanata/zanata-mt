...
while getopts ":fehtg:m:d:l:" opt; do
  case ${opt} in
    h)
    ...
    m)
      ...
      ;;
    l)
      DEEPL_OPTION="-DDEEPL_KEY=$OPTARG"
      ;;
    d)
...
./mvnw docker:build -pl :mt-server -DskipTests && \
  ./mvnw docker:start -pl :mt-server ${GOOGLE_OPTION} ${MS_OPTION} ${DEEPL_OPTION} \
