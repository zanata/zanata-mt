...
timestamps {
  ...
  if (branchName == 'master') {
    ...
    final String DOCKER_IMAGE2 = 'machine-translations/mt-server'
    ...
    lock(resource: 'MT-DEPLOY-TO-STAGE', inversePrecedence: true) {
      ...
      stage('Deploy to STAGE') {
        def tasks = [
                ...
                "DOCKER_BUILD_RELEASE2": {
                  dockerBuildAndDeploy2("$DOCKER_IMAGE2")
                  deployToStage2("$DOCKER_IMAGE2")
                }
        ]
        parallel tasks
      }
 ...
 */
void dockerBuildAndDeploy2(String dockerImage) {
  final String DOCKER_WORKSPACE = 'server/target/docker_workspace2'
  node(defaultNodeLabel) {
    echo "running on node ${env.NODE_NAME}"
    checkout scm
    // Clean the workspace
    ...
    sh "docker tag $dockerImage:$version $MT_DOCKER_REGISTRY_URL2/$dockerImage:$version"
    sh "docker tag $dockerImage:$version $MT_DOCKER_REGISTRY_URL2/$dockerImage:latest"

    echo "Docker login.."
    withCredentials([string(credentialsId: 'MT-DOCKER-REGISTRY-TOKEN',
            variable: 'MT_REGISTRY_TOKEN')]) {
      sh "docker login -p $MT_REGISTRY_TOKEN -e unused -u unused $MT_DOCKER_REGISTRY_URL2"

      echo "Pushing to docker registry..."
      sh "docker push $MT_DOCKER_REGISTRY_URL2/$dockerImage:$version"
      sh "docker push $MT_DOCKER_REGISTRY_URL2/$dockerImage:latest"

      echo "Docker logout.."
      sh "docker logout $MT_DOCKER_REGISTRY_URL2"

      echo "Remove local docker image $dockerImage:$version"
      sh "docker rmi $dockerImage:$version; docker rmi $MT_DOCKER_REGISTRY_URL2/$dockerImage:$version; docker rmi $MT_DOCKER_REGISTRY_URL2/$dockerImage:latest"
    }
  }
}

/**
 * TODO: replace with dockerBuildAndDeploy2()
 ...
 * @param dockerImage
 */
void deployToStage2(String dockerImage) {
  if (currentBuild.result == null || currentBuild.result == 'SUCCESS') {
    node(defaultNodeLabel) {
      def ocClient = getOpenshiftClient();
      ...
      withCredentials(
        ...
        sh "$ocClient login $MT_OPENSHIFT_URL2 --username=$USERNAME --password=$PASSWORD --insecure-skip-tls-verify --namespace=$MT_STAGE_PROJECT_NAME2"

        echo "Update 'latest' tag to $version"
        sh "$ocClient tag $MT_DOCKER_REGISTRY_URL2/$dockerImage:$version mt-server:latest"
        sh "$ocClient logout"
      }
    }
  }
